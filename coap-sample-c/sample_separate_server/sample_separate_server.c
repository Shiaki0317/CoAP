...
void separate_handler(coap_resource_t *resource,coap_session_t *session, const coap_pdu_t *request, const coap_string_t *query, coap_pdu_t *response) {
...
int main(void) {
  ...
  resource = coap_resource_init(coap_make_str_const("sample_data"), 0);
  coap_register_handler(resource, COAP_REQUEST_GET, separate_handler);
